...
```env
# Supabase
SUPABASE_URL=https://ifimawfcqooucjktmcop.supabase.co
...
The frontend Supabase anon key is hardcoded in `src/lib/supabase.js` (safe — it's a public key with RLS enforcing access). The service role key is only used server-side inside Netlify Functions.
